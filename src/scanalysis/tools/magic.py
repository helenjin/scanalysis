...
def run_magic(data, n_pca_components=20, random_pca=True,
          t=6, k=30, ka=10, epsilon=1, rescale=99):

    if n_pca_components != None:
        print('Doing PCA')
        pca_projected_data, b = run_pca(data, n_components=n_pca_components, random=random_pca)
        print('Using pca_projections')
    else:
    ...
    new_data = pd.DataFrame(new_data, index=data.index, columns=['MAGIC ' + gene for gene in data.columns.values])
    return new_data
