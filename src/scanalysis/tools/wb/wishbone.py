...
from scanalysis.plots.wb_plot import WBResults
...
def run_wishbone(data, start_cell, k=15, l=15, num_graphs=1, num_waypoints=250, 
        ...
        for graph_iter in range(num_graphs):
                ...
                results.trajectory = pd.Series(trajectory1, index=data_.index)
                
                results.branch = None
                if branch:
                        results.branch = pd.Series([np.int(i) for i in branches], index=data_.index)
                results.waypoints = list(data_.index[waypoints])
