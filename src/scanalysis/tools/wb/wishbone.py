...
class Wishbone:
        ...
        def run_wishbone(self, df, start_cell, k=15, l=15, num_graphs=1, num_waypoints=250, 
                ...
                """
                :df: can be pandas DataFrame or numpy array object
                :start_cell: can be string (name of start cell) or position (row?) in data matrix
                ...
                if isinstance(start_cell, str):
                        s = np.where(df.index == start_cell)[0]
                        if len(s) == 0:
                        ...
                        if isinstance(num_waypoints, list):
                                if len(pd.Index(num_waypoints).difference(df.index)) > 0:
                ...
                for graph_iter in range(num_graphs):
                        if k!=l:
                                klnn = self._spdists_klnn(lnn, k, verbose)
                        else:
                                klnn = lnn
                        ...
                        self.trajectory.append(iter_traj)
                        self.waypoints.append(iter_l)
                        
                        if branch:
                                # Recalculate branches post reassignments
                                RNK, bp, diffdists, Y = self._splittobranches(traj, traj[0], data, iter_l, dist,paths_l2l)
                                self.branches.append(RNK)
                                self.bas.append(Y)
