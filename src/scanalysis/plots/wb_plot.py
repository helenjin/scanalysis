...
def plot_wishbone_on_tsne(Wishbone, tsne):
 ...
 #       raise RuntimeError('Please run tSNE using scdata.run_tsne before plotting')
    ...
    plt.scatter(tsne['x'], tsne['y'],
        edgecolors='none', s=size, cmap=cmap, c=Wishbone.trajectory)
...
def plot_marker_trajectory(data, Wishbone, markers, show_variance=False,
...
#        raise RuntimeError('Please run Wishbone run_wishbone before plotting')
    ...
    if Wishbone.branch is not None:
        ...
        # Branch of the trunk
        trunk = Wishbone.branch[trajectory.index[0]]
        branches = list( set( Wishbone.branch).difference([trunk]))
        ...
        for j in branch_counts.columns:
            branch_counts[j] = pd.Series([sum(Wishbone.branch[trajectory.index[(trajectory > bins[i-1]) & \
    ...
    for marker,color in zip(markers, colors):
        ...
        if not plot_branch:
            ...
            for br in branches:
                ...
                weights = weights_copy.copy()
                weights.ix[Wishbone.branch.index[Wishbone.branch == br], :] = 0
            ...
            if sum( abs(pd.Series(branch_vals[0]) - pd.Series(branch_vals[1])) > min_delta ) < 5:
                ...
                for br_ind,br in enumerate(branches):
                    ...
                    for i in range(smooth_bins):
                        weights.ix[Wishbone.branch == br, bp_bin + i - smooth_bins] *= ((smooth_bins - i)/smooth_bins) * 0.25
                    weights.ix[Wishbone.branch == br, (bp_bin):weights.shape[1]] = 0
...
def plot_marker_heatmap(Wishbone, marker_trends, trajectory_range=[0, 1]):
...
def plot_derivatives(Wishbone, marker_trends, trajectory_range=[0, 1]):
